...
all = gene_selection %>% 
ggplot(.) +
  geom_point(aes(x = sqrt_mt, y = pLoF), color = "red", show.legend = F) + 
...
ggplot(.) +
  geom_point(aes(x = sqrt_mt, y = pLoF), color = "blue", show.legend = F) + 
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "black") +
...
```{r}
# Let's just plot this with all genes separating the color if it's macarthur_rec or not
all = gene_selection %>% 
ggplot(.) +
  geom_point(aes(x = s_het, y = pLoF, colour = factor(macarthur_rec))) + 
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "black") +
...
# Let's show just macarthur_rec
macarthur_rec = gene_selection %>% filter(macarthur_rec == 1) %>% 
ggplot(.) +
  geom_point(aes(x = s_het, y = pLoF, colour = factor(macarthur_rec))) + 
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "black") +
  labs(title = "Observed LoF Mutations vs Heterozygous Selection",
