...
I will also include a new variable to define "pure" vs "not" vs "mixed" recessive status based on these analytical methods and plots that I will have
...
omim = gene_selection %>% 
  dplyr::select(symbol, pLoF_muL, pLoF_shet, recessive, rec_lethal, analytical_recessive) %>% unique() %>% 
...
# Plotting mutational target vs pLoF

If I assume that the recessive genes only affect homozygotes then they are bounded by 1 when we model squareroot mutational target vs pLoF. If points fall below y=x then there is a non-recessive effect, likely in a quantitative trait
...
Make the necessary plotting datasets or at least pull the necessary and unique rows
```{r}
plotting_df = gene_selection %>% 
  select(sqrt_mt, pLoF, pLoF_muL, recessive, rec_lethal, symbol) %>% 
  unique()
```
...
```{r}
# Let's just plot this with all genes separating the color if it's recessive or not
all = gene_selection %>% 
ggplot(.) +
  geom_point(aes(x = sqrt_mt, y = pLoF, colour = factor(recessive))) + 
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "black") +
  labs(title = "Expected vs Observed LoF mutations",
...
# Let's show just recessive
recessive = gene_selection %>% filter(recessive == 1) %>% 
ggplot(.) +
  geom_point(aes(x = sqrt_mt, y = pLoF, colour = factor(recessive))) + 
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "black") +
  labs(title = "Expected vs Observed LoF mutations",
...
I also have a variable that indicates if the gene is recessive lethal. I want to make more of these plots where I now show these recessive lethal genes and compare that to recessive genes that are not lethal

```{r}
# Plotting recessive genes and differentiating based on recessive lethal
recessive_only = gene_selection %>% filter(recessive == 1) %>% 
ggplot(.) +
...
# now plotting only the recessive lethal genes
...
I also want to create a set of plots with labels of the genes. I'm only going to do this for the last 2 plots I made of just recessive genes
...
# Plotting Shet vs bias-corrected effect sizes

Alright, now I want to plot out the association between Shet and effect sizes. Theoretically, all the recessive genes will have Shet = 0, but they will probably have Shet > 0 due to how GeneBayes work and because we assume there are het effects.

However, we expect this curve and the burden test effect sizes to be lower for the recessive genes compared to the universe of all genes
...
plotting_df = burden_tests %>% 
  select(ensg, s_het, bc_effect_sqr, recessive, rec_lethal) %>% 
  unique() %>% na.omit() %>% 
  # These results are super noisy so I need to create bins of my genes. I want to bin on recessive status before anything 
  arrange(s_het) %>%
  group_by(recessive) %>% 
  mutate(bin = cut_number(s_het, n = 100, labels = FALSE)) %>% 
  # Now group on bin along with the gene statuses to calculate shet and effect
  group_by(bin, recessive, rec_lethal) %>% 
  mutate(bin_shet = mean(s_het),
         bin_effect = mean(bc_effect_sqr)) %>% 
  ungroup() %>% 
  # And now pull realy the only columns I need for plotting
  select(bin, bin_shet, bin_effect, recessive, rec_lethal) %>%
  unique()
```
...
With this information, I can now plot this. I will do similar to the previous set of plots where I will look at all genes, then recessive genes, and then recessive lethal genes

```{r}
# First all genes
all_genes = plotting_df %>% 
ggplot(aes(x = bin_shet, y = bin_effect, colour = factor(recessive))) +
  geom_point() + 
...
# And then only look at recessive genes
all_recessive = plotting_df %>% filter(recessive == 1) %>% 
ggplot(aes(x = bin_shet, y = bin_effect, colour = factor(recessive))) +
  geom_point() + 
  geom_smooth( method = "loess", se = FALSE) +
  labs(title = "Gene Selection vs Observed Effect Size",
...
# And now look at separation between recessive and recessive lethal
recessive_plus_lethal = plotting_df %>% filter(recessive == 1) %>% 
...
# And then just recessive lethal genes
...
all = gene_selection %>% 
ggplot(.) +
  geom_point(aes(x = s_het, y = pLoF, colour = factor(recessive))) + 
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "black") +
  labs(title = "Observed LoF Mutations vs Heterozygous Selection",
       x = "S_het",
       y = "pLof",
       color = "Recessive Gene?") +
  xlim(0, 0.01) +
...
I also want to make another version of these plots where I label the recessive genes where a majority of the selection is on homozygotes. This means genes that fall above the line

I want to do this only for the all recessive genes plot and the recessive letahl genes plot

```{r}
# Plotting recessive genes and differentiating based on recessive lethal WITH LABELS
recessive_labeled = gene_selection %>% filter(recessive == 1) %>% 
ggplot(.) +
  geom_point(aes(x = s_het, y = pLoF, colour = factor(rec_lethal))) + 
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "black") + 
  geom_text_repel(
      data = filter(gene_selection, recessive == 1, pLoF_shet > 1),
...
# now plotting only the recessive lethal genes With LABELS
...
```{r}
# Let's define pure and not pure recessive genes for both the recessive lethal and not recessive lethal genes
...
# Now, find the disorders/diseases associated with these pure recessive genes
omim_pure <- omim %>%
...
# And then the disorders/diseases associated not pure recessive genes
omim_not <- omim %>%
  filter(map_lgl(str_split(`gene`, ",\\s*"), 
                 ~ any(.x %in% not_genes))) %>%
  mutate(gene = map_chr(str_split(`gene`, ",\\s*"),
...
Now that I have a list of "Pure" and "Not Pure" recessive genes, I want to analyze the function and pathways related to these genes. To do that I will perform GO and KEGG enrichment on my "Pure" and "Not Pure" recessive genes segregated by "Recessive Lethal" status
...
pure_entrez = gene_selection %>% 
  filter(analytical_recessive == "Pure", recessive == 1) %>% 
  pull(ensg) %>% 
...
pure_entrez_lethal = gene_selection %>% 
  filter(analytical_recessive == "Pure", recessive == 1, rec_lethal == 1) %>% 
  pull(ensg) %>% 
  AnnotationDbi::select(org.Hs.eg.db, 
                        keys = ., 
                        columns = "ENTREZID", 
...
not_entrez = gene_selection %>% 
  filter(analytical_recessive == "Not", recessive == 1) %>% 
  pull(ensg) %>% 
  AnnotationDbi::select(org.Hs.eg.db, 
                        keys = ., 
                        columns = "ENTREZID", 
...
not_entrez_lethal = gene_selection %>% 
  filter(analytical_recessive == "Not", recessive == 1, rec_lethal == 1) %>% 
  pull(ensg) %>% 
  AnnotationDbi::select(org.Hs.eg.db, 
                        keys = ., 
                        columns = "ENTREZID", 
...
Something I'm immediately worried about is that these enrichments (especially the lysosome results seen in the "Not Pure" category) are biased by the fact that most Recessive genes fall within those categories. I want to run 1 set of GO and KEGG enrichment on all recessive genes just so I have a baseline understanding of enrichment
...
# All recessive GO and KEGG enrichment
...
The "All Recessive Genes" is basically my control over the whole space of genes I'm looking at. And then I split on "Pure' and "Not Pure". And then I split again based on if they are "recessive lethal" or not

```{r}
# First all the recessive gene names
...
pure_names = gene_selection %>% 
  filter(recessive == 1, analytical_recessive == "Pure") %>% 
  pull(symbol) 

# Then Pure Lethal Genes
pure_lethal_names = gene_selection %>% 
  filter(recessive == 1, analytical_recessive == "Pure", rec_lethal == 1) %>% 
  pull(symbol) 

# Then Not Pure Genes
not_names = gene_selection %>% 
  filter(recessive == 1, analytical_recessive == "Not") %>% 
  pull(symbol) 

# Then Not Pure Lethal Genes
not_lethal_names = gene_selection %>% 
  filter(recessive == 1, analytical_recessive == "Not", rec_lethal == 1) %>% 
  pull(symbol) 
...
# Enrichment on all recessive diseases
...
```{r}
# I will save my results as TSVs
# All recessive phenotype enrichment
